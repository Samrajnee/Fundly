    ...
    raw = await callGeminiTool({
      ...
      schema: {
        ...
        properties: {
          ...
          merchant: { type: SchemaType.STRING, description: "Merchant or place name if mentioned" },
          date: { type: SchemaType.STRING, description: "ISO date (YYYY-MM-DD) the expense occurred on" },
          suggestedCategoryName: { type: SchemaType.STRING, description: "Best matching category name from the provided list" },
confidence: { type: SchemaType.STRING, format: "enum", enum: ["HIGH", "MEDIUM", "LOW"], description: "How confident the extraction is" },        },
